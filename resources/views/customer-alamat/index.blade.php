    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-10">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                <div>
                    <h1 class="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Pengurusan Alamat Prospek</h1>
                    <p class="text-gray-600">Urus alamat prospek anda</p>
                </div>
                <a href="{{ route('prospek-alamat.create') }}"
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                        <tr>
                            <th class="px-6 py-4 text-left text-sm font-semibold text-gray-700">Prospek</th>
                                ...
                                <tr class="hover:bg-amber-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex space-x-2">
                                            <a href="{{ route('prospek-alamat.show', $item) }}"
                                            ...
                                            <a href="{{ route('prospek-alamat.edit', $item) }}"
                                            ...
                                            <form method="POST" action="{{ route('prospek-alamat.destroy', $item) }}"
                                                class="inline"
                ...
                <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-gray-700">
                            Menunjukkan <span class="font-medium">{{ $prospekAlamat->firstItem() }}</span> ke <span
                                class="font-medium">{{ $prospekAlamat->lastItem() }}</span> daripada <span
                                class="font-medium">{{ $prospekAlamat->total() }}</span> rekod
                        </div>
                        <div>
                            {{ $prospekAlamat->links('vendor.pagination.tailwind') }}
                        </div>
                    </div>
                </div>
            @endif
