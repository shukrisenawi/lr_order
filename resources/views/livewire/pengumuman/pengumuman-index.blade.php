<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-10">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
                <h1 class="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Pengurusan Pengumuman</h1>
                <p class="text-gray-600">Urus pengumuman dengan tarikh</p>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                    <tr>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-gray-700">Pengumuman</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($pengumumen as $item)
                        <tr class="hover:bg-blue-50 transition-colors duration-200"
                            wire:key="pengumuman-{{ $item->id }}">
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">{{ Str::limit($item->pengumuman, 100) }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex space-x-2">
                                    ...
                                    <button wire:click="delete({{ $item->id }})"
                                        wire:confirm="Adakah anda pasti mahu memadamkan pengumuman ini?"
                        ...
                        <tr>
                            <td colspan="3" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 mb-1">Tiada pengumuman dijumpai</h3>
                                    <p class="text-gray-500 mb-4">Cuba ubah carian atau tambah pengumuman baru</p>
                                    <a href="{{ route('pengumuman.create') }}"
                                        ...
                                        <i class="fas fa-plus mr-2"></i>
                                        Tambah pengumuman pertama anda
                                    </a>
                                </div>
                            </td>
                        </tr>
            ...
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-700">
                        Menunjukkan <span class="font-medium">{{ $pengumumen->firstItem() }}</span> ke <span
                            class="font-medium">{{ $pengumumen->lastItem() }}</span> daripada <span
                            class="font-medium">{{ $pengumumen->total() }}</span> rekod
                    </div>
                    <div class="flex space-x-1">
                        ...
                        @else
                            <button wire:click="gotoPage({{ $pengumumen->currentPage() - 1 }})"
                        ...
                        @php
                            $start = max(1, $pengumumen->currentPage() - 2);
                            $end = min($pengumumen->lastPage(), $pengumumen->currentPage() + 2);
                        @endphp

                        @for ($page = $start; $page <= $end; $page++)
                            @if ($page == $pengumumen->currentPage())
