<div class="w-full px-2 sm:px-4 lg:px-6 py-4 min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 via-purple-50 to-pink-50">
    ...
    <div class="mb-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-900 mb-2">Create by AI Approval</h1>
                <p class="text-sm text-gray-600">Review and approve items created by AI</p>
            ...
            <div class="flex gap-2">
                @if($activeTab === 'customers' && $customers->count() > 0)
                    <button wire:click="approveAllCustomers"
                        class="inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-medium rounded-lg shadow-md hover:from-green-600 hover:to-emerald-700 transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
                ...
                <table class="w-full">
                    <thead class="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700">No Tel</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($customers as $customer)
                            <tr class="hover:bg-blue-50 transition-colors duration-200" wire:key="customer-{{ $customer->id }}">
                                <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $customer->gelaran }} {{ $customer->nama_penerima }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600">{{ $customer->alamat }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600">{{ $customer->no_tel }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600">{{ $customer->email ?: '-' }}</td>
                                <td class="px-4 py-3 text-sm text-gray-500">{{ $customer->created_at->format('d M Y H:i') }}</td>
                                <td class="px-4 py-3">
                                    <div class="flex gap-1">
                                        <a href="{{ route('customer.show', $customer) }}?from=ai&tab={{ $activeTab }}"
                                            class="inline-flex items-center px-2 py-1 text-sm font-medium text-amber-700 bg-amber-50 rounded hover:bg-amber-100 transition-colors duration-200">
                                            <i class="fas fa-eye mr-1"></i>
                                            View
                                        </a>
                                        <button wire:click="approveCustomer({{ $customer->id }})"
                                            class="inline-flex items-center px-3 py-1 text-sm font-medium text-white bg-gradient-to-r from-blue-500 to-blue-600 rounded hover:from-blue-600 hover:to-blue-700 transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                            ...
                            <tr>
                                <td colspan="6" class="px-4 py-8 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <h3 class="text-base font-medium text-gray-900 mb-1">No pending customers</h3>
                                        <p class="text-sm text-gray-500">All customers have been approved.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                        <tr>
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700">Invoice No</th>
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700">Nama Penerima</th>
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700">Alamat</th>
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700">No Tel</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($invoices as $invoice)
                            <tr class="hover:bg-green-50 transition-colors duration-200" wire:key="invoice-{{ $invoice->id }}">
                                <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $invoice->invoice_no }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600">{{ $invoice->nama_penerima }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600">{{ $invoice->alamat }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600">{{ $invoice->no_tel }}</td>
                                <td class="px-4 py-3 text-sm font-medium text-green-600">RM {{ number_format($invoice->jumlah, 2) }}</td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-500">{{ $invoice->created_at->format('d M Y H:i') }}</td>
                                <td class="px-4 py-3">
                                    <div class="flex gap-1">
                                        <a href="{{ route('invoice.show', $invoice) }}?from=ai&tab={{ $activeTab }}"
                            ...
                            <tr>
                                <td colspan="8" class="px-4 py-8 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <h3 class="text-base font-medium text-gray-900 mb-1">No pending invoices</h3>
                                        <p class="text-sm text-gray-500">All invoices have been approved.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
