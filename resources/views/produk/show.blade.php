    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                <div>
                    <h1 class="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Lihat Produk</h1>
                    <p class="text-gray-600">Maklumat terperinci untuk {{ $produk->name ?? 'N/A' }}</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <!-- Product Name -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-800 mb-2">Nama Produk</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl">
                            <p class="text-gray-900">{{ $produk->name ?? 'N/A' }}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl">
                            <p class="text-gray-900 font-medium">RM {{ number_format($produk->price ?? 0, 2) }}</p>
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl">
                            <p class="text-gray-900">{{ $produk->description ?? 'Tiada keterangan' }}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl">
                            <p class="text-gray-900">{{ $produk->stok ?? 'N/A' }}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl">
                            @if ($produk->gambar)
                                <p class="text-gray-900">{{ $produk->gambar->nama ?? 'N/A' }}</p>
                            @else
                ...
                <div class="mt-8 flex flex-col sm:flex-row justify-end space-y-4 sm:space-y-0 sm:space-x-4">
                    ...
                    <form method="POST" action="{{ route('produk.destroy', $produk) }}" class="inline"
                        onsubmit="return confirm('Adakah anda pasti mahu memadamkan produk ini?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
