    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @foreach($userBisnes as $bisnes)
                <div class="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 border border-gray-200 overflow-hidden">
                    <div class="p-6">
                        <!-- Company Logo -->
                        <div class="flex items-center justify-center mb-4">
                            ...
                            @else
                                <div class="w-16 h-16 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 flex items-center justify-center shadow-lg">
                                    <span class="text-white font-bold text-xl">{{ strtoupper(substr($bisnes->nama_bisnes, 0, 1)) }}</span>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-4 mb-6 text-center">
                            ...
                            <div>
                                <div class="text-2xl font-bold text-green-600">{{ \App\Models\Customer::where('bisnes_id', $bisnes->id)->count() }}</div>
                                <div class="text-xs text-gray-500">Pelanggan</div>
                            </div>
                            <div>
                                <div class="text-2xl font-bold text-purple-600">{{ \App\Models\Invoice::where('bisnes_id', $bisnes->id)->count() }}</div>
